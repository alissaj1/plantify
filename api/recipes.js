  ...
  try {
    ...
    if (!upstream.ok) {
      ...
      return res.status(502).json({
        error: `OpenAI request failed with status ${upstream.status} (${upstream.statusText})`,
      });
    }
    ...
    try {
      ...
      console.error('Plantify upstream JSON parse error:', err);
    ...
    console.error('Plantify API function error:', error);
    return res
      .status(500)
      .json({ error: 'Server error while generating recipes.' });
  }
};
